...
class Article(models.Model):
    city = models.ForeignKey(City, related_name='articles', on_delete=models.CASCADE, db_index=True)
    ...
    author = models.CharField(max_length=100, blank=True, db_index=True)  # Frequently filtered
    ...
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)  # Frequently ordered by
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        indexes = [
            models.Index(fields=['status', 'created_at']),  # Common filter + order combination
