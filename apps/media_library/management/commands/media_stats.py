...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--storage", action="store_true", help="Show storage information"
        )
        parser.add_argument(
            "--usage-report",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Media Library Statistics"))
        ...
        self.stdout.write(
            f"Total Size: {MediaUtils.format_file_size(stats['total_size'])}"
        )
        ...
        if detailed:
            ...
            for days in [1, 7, 30]:
                ...
                self.stdout.write(
                    f"  Last {days} day{'s' if days > 1 else ''}: {count}"
                )
            ...
            for file_type in ["image", "video", "document"]:
                ...
                if queryset.exists():
                    ...
                    if count > 0:
                        ...
                        self.stdout.write(
                            f"  {file_type.capitalize()}: {MediaUtils.format_file_size(int(avg_size))}"
                        )
        ...
        if show_storage:
            ...
            if "error" in storage_info:
                self.stdout.write(self.style.ERROR(f'Error: {storage_info["error"]}'))
            else:
                self.stdout.write(f"Media Root: {storage_info['media_root']}")
                self.stdout.write(f"Files in Storage: {storage_info['total_files']}")
                self.stdout.write(
                    f"Storage Size: {MediaUtils.format_file_size(storage_info['total_size'])}"
                )

                if storage_info["disk_total"]:
                    self.stdout.write(
                        f"Disk Total: {MediaUtils.format_file_size(storage_info['disk_total'])}"
                    )
                    self.stdout.write(
                        f"Disk Used: {MediaUtils.format_file_size(storage_info['disk_used'])}"
                    )
                    self.stdout.write(
                        f"Disk Free: {MediaUtils.format_file_size(storage_info['disk_free'])}"
                    )
                    self.stdout.write(
                        f"Disk Usage: {storage_info['disk_usage_percentage']:.1f}%"
                    )
        ...
        if usage_report:
            ...
            for activity in report["recent_uploads"]:
                self.stdout.write(
                    f"  {activity['period']}: {activity['count']} uploads"
                )
            ...
            for file_type, count in report["file_type_distribution"].items():
                ...
                self.stdout.write(
                    f"  {file_type.capitalize()}: {count} ({percentage:.1f}%)"
                )
