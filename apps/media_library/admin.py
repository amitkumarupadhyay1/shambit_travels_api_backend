...
class MediaAdmin(admin.ModelAdmin):
    list_display = [
        'thumbnail_preview', 'title_with_filename', 'file_type_badge', 
        'file_size_display', 'content_object_link', 'created_at'
    ]
    ...
    fieldsets = (
        ('Media File', {
            'fields': ('file', 'file_preview', 'file_info_display')
        }),
        ('Media Info', {
    ...
    def optimize_images(self, request, queryset):
        ...
        self.message_user(
            request,
            f'Optimized {optimized_count} image files.',
            messages.SUCCESS
        )
    ...
    def media_dashboard_view(self, request):
        ...
        recent_media = Media.objects.filter(created_at__gte=recent_date).order_by('-created_at')[:10]
