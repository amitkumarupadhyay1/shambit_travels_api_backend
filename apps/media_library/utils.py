...
class MediaValidator:
    ...
    def _validate_file_size(self, file, file_info: Dict[str, Any]):
        ...
        if file_type == 'image' and file_size > self.MAX_IMAGE_SIZE:
            raise ValueError(f"Image file too large. Maximum size is {self.MAX_IMAGE_SIZE // (1024*1024)}MB")
    ...
    def _validate_file_type(self, file, file_info: Dict[str, Any]):
        ...
        if extension not in all_allowed:
            raise ValueError(f"File type not allowed. Allowed extensions: {', '.join(all_allowed)}")
        ...
        try:
            ...
            with Image.open(file) as img:
                ...
                if width > max_width or height > max_height:
                    raise ValueError(f"Image dimensions too large. Maximum: {max_width}x{max_height}")
                ...
                if width < min_width or height < min_height:
                    raise ValueError(f"Image dimensions too small. Minimum: {min_width}x{min_height}")
...
class MediaProcessor:
    ...
    def _process_image(self, media: Media) -> Dict[str, Any]:
        ...
        try:
            with Image.open(media.file.path) as img:
                ...
                if img.mode == 'RGBA' and media.file.name.lower().endswith(('.jpg', '.jpeg')):
    ...
    def generate_thumbnail(self, media: Media, width: int = 150, height: int = 150) -> Optional[str]:
        """
        ...
        try:
            ...
            thumbnail_dir = os.path.join(
                os.path.dirname(media.file.path),
                'thumbnails'
            )
            ...
            relative_path = os.path.relpath(thumbnail_path, getattr(settings, 'MEDIA_ROOT', ''))
    ...
    def optimize_media(self, media: Media) -> Dict[str, Any]:
        ...
        if file_type == 'image':
            ...
            return {'success': False, 'error': 'Optimization not supported for this file type'}
