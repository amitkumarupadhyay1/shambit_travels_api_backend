...
class MediaModelTest(TestCase):
    ...
    def test_media_without_title_uses_filename(self):
        media = Media.objects.create(
            file=self.test_image, content_type=self.content_type, object_id=self.user.id
        )
...
class MediaAPITest(APITestCase):
    ...
    def create_test_image(self):
        ...
        return SimpleUploadedFile(
            name="api_test.jpg", content=image_io.getvalue(), content_type="image/jpeg"
        )
    ...
    def test_update_media_metadata(self):
        ...
        url = reverse("media-detail", kwargs={"pk": self.media.pk})
        data = {"title": "Updated Title", "alt_text": "Updated alt text"}
