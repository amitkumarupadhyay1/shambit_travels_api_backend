...
class PricingService:
    ...
    def get_price_breakdown(package, experiences, hotel_tier, transport_option):
        ...
        base_experience_total = sum(Decimal(str(exp.base_price)) for exp in experiences) if experiences else Decimal('0.00')
    ...
    def get_applicable_rules(package):
        ...
        active_rules = PricingRule.objects.filter(
            ...
            Q(target_package=package) | Q(target_package__isnull=True)
        ).order_by('active_from')  # Apply rules in chronological order
        
        # Cache for 5 minutes
        cache.set(cache_key, list(active_rules), 300)
        
        logger.info(f"Applied {len(active_rules)} pricing rules for package {package.slug if package else 'global'}")
    ...
    def validate_price_components(package, experiences, hotel_tier, transport_option):
        ...
        if hotel_tier.id not in package.hotel_tiers.values_list('id', flat=True):
            errors.append(f"Hotel tier {hotel_tier.id} not available for package {package.slug}")
        
        # Check if transport option belongs to package
        if transport_option.id not in package.transport_options.values_list('id', flat=True):
            errors.append(f"Transport option {transport_option.id} not available for package {package.slug}")
    ...
    def get_price_estimate_range(package):
        ...
        try:
            ...
            cheapest_transport = package.transport_options.order_by('base_price').first()
            ...
            most_expensive_experiences = package.experiences.order_by('-base_price')[:3]  # Assume max 3 experiences
            most_expensive_hotel = package.hotel_tiers.order_by('-price_multiplier').first()
            most_expensive_transport = package.transport_options.order_by('-base_price').first()
            
            max_price = PricingService.calculate_total(
                package, most_expensive_experiences, most_expensive_hotel, most_expensive_transport
            )
            ...
            logger.error(f"Error calculating price range for package {package.slug}: {str(e)}")
            return {
