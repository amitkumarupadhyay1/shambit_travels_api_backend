...
class SEOService:
    ...
    @staticmethod
    def create_seo_data(
        content_type: str, object_id: int, seo_data: Dict[str, Any]
    ) -> SEOData:
        """
        ...
        existing_seo = SEOData.objects.filter(
            content_type=ct, object_id=object_id
        ).first()
        if existing_seo:
            raise ValueError(
                f"SEO data already exists for {content_type} with id {object_id}"
            )

        return SEOData.objects.create(content_type=ct, object_id=object_id, **seo_data)
    ...
    @staticmethod
    def get_or_create_seo_data(
        content_type: str, object_id: int, defaults: Dict[str, Any]
    ) -> tuple:
        """
    ...
    @staticmethod
    def bulk_create_seo_data(
        content_type: str, object_ids: List[int], seo_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        seo_objects = [
            SEOData(content_type=ct, object_id=obj_id, **seo_data) for obj_id in new_ids
        ]
    ...
    def bulk_update_seo_data(
        ...
        seo_objects = SEOData.objects.filter(content_type=ct, object_id__in=object_ids)
        ...
        return {"updated_count": updated_count, "total_requested": len(object_ids)}
    ...
    def generate_seo_from_content(content_type: str, object_id: int) -> SEOData:
        ...
        seo_obj, created = SEOData.objects.get_or_create(
            content_type=ct, object_id=object_id, defaults=seo_data
        )
    ...
    def _generate_seo_for_object(obj) -> Dict[str, Any]:
        ...
        if hasattr(obj, "title") and hasattr(obj, "description"):
            ...
            seo_data["description"] = (
                obj.description[:500] if len(obj.description) > 500 else obj.description
            )
            ...
            if hasattr(obj, "city"):
                seo_data["keywords"] = (
                    f"{obj.name}, {obj.city.name}, travel package, vacation"
                )
    ...
    def get_seo_stats() -> Dict[str, Any]:
        ...
        by_content_type = (
            SEOData.objects.values("content_type__app_label", "content_type__model")
        ...
        return {
            ...
            "completeness": {
                ...
                "basic_seo_percentage": (
                    (complete_seo / total_seo_data * 100) if total_seo_data > 0 else 0
                ),
                "og_percentage": (
                    (with_og_data / total_seo_data * 100) if total_seo_data > 0 else 0
                ),
                "structured_data_percentage": (
                    (with_structured_data / total_seo_data * 100)
                    if total_seo_data > 0
                    else 0
                ),
            },
    ...
    def generate_sitemap_data(content_type: str) -> List[Dict[str, Any]]:
        ...
        seo_objects = SEOData.objects.filter(content_type=ct).select_related(
            "content_type"
        )
            ...
            sitemap_data.append(
                {
                    ...
                    "last_modified": getattr(
                        content_obj,
                        "updated_at",
                        getattr(content_obj, "created_at", None),
                    ),
    ...
    def seo_health_check() -> Dict[str, Any]:
        ...
        return {
            ...
            "health_percentage": (
                (good_count / len(all_seo_data) * 100) if all_seo_data else 0
            ),
