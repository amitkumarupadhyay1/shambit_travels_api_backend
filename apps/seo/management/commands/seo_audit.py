...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--export',
            type=str,
            help='Export report to file (CSV format)'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Starting SEO Audit...')
        )
        ...
        self.stdout.write(f"Good SEO: {health_report['good_count']} ({health_report['health_percentage']:.1f}%)")
        ...
        self.stdout.write(f"Complete basic SEO: {completeness['complete_basic_seo']} ({completeness['basic_seo_percentage']:.1f}%)")
        self.stdout.write(f"With Open Graph: {completeness['with_open_graph']} ({completeness['og_percentage']:.1f}%)")
        self.stdout.write(f"With Structured Data: {completeness['with_structured_data']} ({completeness['structured_data_percentage']:.1f}%)")
        ...
        for ct_data in stats['by_content_type']:
            self.stdout.write(f"{ct_data['content_type__app_label']}.{ct_data['content_type__model']}: {ct_data['count']}")
        ...
        if detailed:
            ...
            for seo_data in queryset[:20]:  # Limit to 20 for readability
                ...
                self.stdout.write(f'  Title: {analysis["title_length"]} chars ({analysis["title_score"]})')
                self.stdout.write(f'  Description: {analysis["description_length"]} chars ({analysis["description_score"]})')
                self.stdout.write(f'  Keywords: {analysis["keywords_count"]} ({analysis["keywords_score"]})')
                self.stdout.write(f'  Open Graph: {analysis["og_completeness"]*100:.0f}% complete')
    ...
    def export_to_csv(self, filename, stats, health_report):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            # Header
            writer.writerow(['SEO Audit Report', datetime.now().strftime('%Y-%m-%d %H:%M:%S')])
            writer.writerow([])
            ...
            writer.writerow(['Health Percentage', f"{health_report['health_percentage']:.1f}%"])
            writer.writerow([])
            
            # Completeness
            writer.writerow(['Completeness'])
            completeness = stats['completeness']
            writer.writerow(['Complete Basic SEO', f"{completeness['basic_seo_percentage']:.1f}%"])
            writer.writerow(['With Open Graph', f"{completeness['og_percentage']:.1f}%"])
            writer.writerow(['With Structured Data', f"{completeness['structured_data_percentage']:.1f}%"])
            writer.writerow([])
