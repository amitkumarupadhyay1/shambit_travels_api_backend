...
def test_vehicle_suggestions():
    ...
    if response.status_code == 200:
        ...
        for i, solution in enumerate(data['solutions'], 1):
            ...
            for vehicle in solution['vehicles']:
                print(f"    - {vehicle['count']} × {vehicle['name']} ({vehicle['capacity']} seats)")
            print(f"    Total: {solution['total_vehicle_count']} vehicles, {solution['total_capacity']} seats")
            print(f"    Unused: {solution['unused_seats']} seats")
            print(f"    Cost: ₹{solution['total_cost']} (₹{solution['cost_per_day']}/day)")
    ...
    if response.status_code == 200:
        ...
        if data['solutions']:
            ...
            print(f"  Best: {solution['vehicles'][0]['name']} - ₹{solution['total_cost']}")
    else:
        print(f"✗ Error: {response.text}")
