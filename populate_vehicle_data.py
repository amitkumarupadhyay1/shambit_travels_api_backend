...
from packages.models import TransportOption  # noqa: E402
...
def populate_vehicle_data():
    ...
    print("\nActive Vehicles:")
    for transport in TransportOption.objects.filter(is_active=True).order_by(
        "passenger_capacity"
    ):
        print(
            f"  {transport.name}: {transport.passenger_capacity} seats, ₹{transport.base_price_per_day}/day"
        )
