...
def test_basic_optimization():
    ...
    for i, sol in enumerate(solutions):
        print(f"   DEBUG: Solution {i+1}: {[(v.name, c) for v, c in sol.vehicles]}, "
    ...
    # So Sedan should win
    assert best.vehicles[0][0].name == "Sedan", f"Should select Sedan but got {best.vehicles[0][0].name}"
    print(f"   ✓ Best: {best.vehicles[0][0].name} x{best.vehicles[0][1]}, Cost: ₹{best.total_cost}")
    ...
    print(f"   ✓ Best: {best.vehicles[0][0].name} x{best.vehicles[0][1]}, Unused: {best.unused_seats}, Cost: ₹{best.total_cost}")
    ...
    print(f"   ✓ Best: {[(v.name, c) for v, c in best.vehicles]}, Capacity: {best.total_capacity}, Cost: ₹{best.total_cost}")
    ...
    for solution in solutions:
        assert solution.total_capacity >= 20, f"Under-capacity solution found: {solution.total_capacity}"
    ...
    print(f"   ✓ Cost per day: ₹{best.cost_per_day}, Total (3 days): ₹{best.total_cost}")
    ...
    engine = VehicleOptimizationEngine(vehicles_with_inactive, passenger_count=10, num_days=1)
    solutions = engine.optimize()
    for solution in solutions:
    ...
    print(f"   ✓ Only active vehicles used")
    ...
    expected = (Decimal("1000") * 2 + Decimal("1500") * 1) * 3  # (2000 + 1500) * 3 = 10500
