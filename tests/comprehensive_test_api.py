...
def get_token():
    ...
    try:
        ...
        if '---TOKEN_START---' in output:
            token = output.split('---TOKEN_START---')[1].split('---TOKEN_END---')[0].strip()
            return token
...
def test_endpoint(path, method, token, path_params=None):
    ...
    try:
        ...
        return {
            ...
            "status": status,
            "duration": duration,
            "response": response.json() if status == 200 and "application/json" in response.headers.get("Content-Type", "") else None
        }
    except Exception as e:
        print(f"ERROR: {e}")
        return {
...
def run_comprehensive_test():
    ...
    for path, methods in endpoints.items():
        if "get" in methods:
            ...
            safe_posts = ["/api/packages/packages/{slug}/calculate_price/", "/api/pricing/rules/test_pricing/"]
...
def generate_report(results):
    ...
    with open(REPORT_FILE, "w") as f:
        ...
        for r in results:
            ...
            f.write(f"| {r['path']} | {r['method']} | {status_str} | {r['duration']:.2f} |\n")
