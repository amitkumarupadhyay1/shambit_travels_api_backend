...
from packages.models import Package, Experience, HotelTier, TransportOption
...
def test_pricing_engine():
    ...
    # Calculate price
    total_price = PricingService.calculate_total(mumbai_package, experiences, hotel_tier, transport)
    ...
    breakdown = PricingService.get_price_breakdown(mumbai_package, experiences, hotel_tier, transport)
    ...
    for rule in breakdown['applied_rules']:
        ...
        print(f"  • {rule['name']}: {rule['type']} {rule['value']}{symbol} = ₹{rule['amount_applied']}")
    ...
    errors = PricingService.validate_price_components(mumbai_package, experiences, hotel_tier, transport)
    if errors:
    ...
    for tier in mumbai_package.hotel_tiers.all():
        tier_price = PricingService.calculate_total(mumbai_package, experiences, tier, transport)
    ...
    for transport_opt in mumbai_package.transport_options.all():
        transport_price = PricingService.calculate_total(mumbai_package, experiences, budget_hotel, transport_opt)
    ...
    try:
        no_exp_price = PricingService.calculate_total(mumbai_package, [], hotel_tier, transport)
    ...
    try:
        all_exp_price = PricingService.calculate_total(mumbai_package, mumbai_package.experiences.all(), hotel_tier, transport)
    ...
    for i in range(100):
        PricingService.calculate_total(mumbai_package, experiences, hotel_tier, transport)
